...
naive_bayes <- R6Class("naive_bayes",
  public = list(
    ...
    fit = function(x, y) {
      ...
      private$training_set <- cbind(head(x,8),head(y,8))
    ...
    #' including information about the model object.
    #'
    ...
    print = function(...) {
      ...
      # print("Ecart-type : ")
      # print(self$var_)
      stringStatus = ifelse(is.null(private$training_set),"Unfitted","Fitted")
      print(paste(stringStatus,"object of class <naive_bayes>"))
    },
    summary = function(){
      self$print()
      if(!is.null(private$training_set)){
        ...
        print("Prior probas : ")
        print(paste(self$classes,':',self$prior_))
        print("Conditional means : ")
        print(as.data.frame(self$mean_,row.names = self$classes))
        print("Conditional variances : ")
        print(as.data.frame(self$var_,row.names = self$classes))
      }
    }
  ),
  private = list(
